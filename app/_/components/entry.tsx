...
const Entry = async ({ id }: { id: string }) => {
    ...
    return (
        <div className="w-full lg:max-w-sm">
            <div className="h-full flex flex-col p-4 md:pb-9 bg-gradient-to-t mx-1 from-white via-neutral-200/75 via-10% to-white to-50%">
                <div className="flex">
                    ...
                    <a href={record.link} target="feed" className='w-full'><h1 className="title-font self-center font-medium text-gray-700 leading-6 md:text-xl">{record.title}</h1></a>
                </div>
                <div className="my-2 mx-3 grow hidden md:block">
                    <p className="leading-relaxed text-sm"><a href={record.link} target="feed">{record.intro.substring(0, 200)}</a></p>
                </div>
                <div className="self-end w-full flex md:items-end text-xs mt-2">
                    ...
                    <div className="w-full text-end">
                        {record.category && <p className='mr-2 md:mr-0 float-left md:float-none'><Link href={{ pathname: '/find', query: { category: record.category } }}>{record.category}</Link></p>}
                        <p className='text-ellipsis overflow-hidden text-nowrap'><Link className="title-font md:font-medium text-gray-500" href={{ pathname: '/find', query: { site: record.site } }}>{record.site}</Link></p>
                    </div>
                </div>
            </div>
        </div>
