...
const Entry: React.FC<EntryProps> = ({ record }) => {
    ...
    const date = new Date(record.date).toLocaleString('ja-JP', { year:'numeric', month: '2-digit', day: '2-digit' });
    return (
        <>
            ...
            <div className="self-end w-full flex md:items-end">
                ...
                <div className="w-full text-end text-sm">
                    ...
                    <p><Link className="title-font md:font-medium text-gray-500" href={{pathname:'find', query:{site:record.siteName}}}>{record.siteName}</Link></p>
                </div>
            </div>
        </>
    );
